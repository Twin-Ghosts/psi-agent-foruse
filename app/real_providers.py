...
这里用标准库 urllib，换 aiohttp 时只改 _post/_get 两处。
...
async def _http(method, url, headers=None, body=None, timeout=20):
    ...
    用 aiohttp 而非 urllib：方案文档要求全程 anyio + aiohttp，且同步 HTTP 会把
    事件循环卡住——发码要等供应商往返几百毫秒，期间整个服务停摆。
    """
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.request(method, url, data=data,
                ...
                try:
                    ...
                    return resp.status, {"__raw__": raw[:300].decode(
                        "utf-8", "replace")}
...
class ResendProvider:
    ...
    def __init__(self, api_key=None, sender=None, api_base=None):
        ...
        self.sender = sender or os.environ.get("RESEND_FROM",
                                               "onboarding@resend.dev")
        self.api_base = (api_base
                         or os.environ.get("RESEND_API_BASE",
...
class PnvsProvider:
    ...
    **验证码由阿里云托管**：生成、存储、有效期、校验全在其侧，我们零存储。
    ...
    def __init__(self, access_key_id=None, access_key_secret=None,
        ...
        self.sign_name = sign_name or os.environ.get("DYPNS_SIGN_NAME", "")
        self.template_code = (template_code
                              or os.environ.get("DYPNS_TEMPLATE_CODE", ""))
        self.api_url = (api_url
                        or os.environ.get("DYPNS_API_URL",
                                          "https://dypnsapi.aliyuncs.com/"))
        self.api_version = os.environ.get("DYPNS_API_VERSION", "2017-05-25")
        self.calls = 0
