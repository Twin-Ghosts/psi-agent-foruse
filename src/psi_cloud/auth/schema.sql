...
-- ---------------------------------------------------------------- 注册中转凭证
--
-- tempToken（/verify/* 发、/complete 收）**必须落库，不能放进程内字典**。
-- 线上 uvicorn 跑 --workers 2：/verify 落 A 进程写进 A 的字典，/complete 若
-- 被分到 B 进程就查不到，报 temp_token_invalid。实测 9 个并发只有 2 个成功。
--
-- 只存哈希，与 sessions.token_hash 同理：库被读走也无法反推凭证。
