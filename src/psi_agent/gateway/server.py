...
async def create_app(
    ...
    if authm is not None:
        ...
        logger.info(f"Auth enabled, proxying to {authm.endpoint}")
...
# ---------------------------------------------------------------- 认证 (/auth/*)
#
# 这些路由只在 ``--auth-endpoint`` 非空时注册 (见 create_app)。为空时整套认证
# 不加载, 现有本地单用户流程零回归 —— 这是本期改动能安全落地的前提。
#
# Gateway 侧刻意只做**转发 + 本机凭证管理**: 不持任何供应商密钥 (安装包里放
# 阿里云 AK/SK 或 Resend key 等于公开发布), 不做授权判定 (用户本人即机器管理员,
# 客户端侧校验可被绕过)。发码与鉴权都在云端。
...
async def _auth_bind(request: web.Request) -> web.Response:
    """已登录态下把手机号/邮箱绑定到当前账号 (R2)。复用发码, 校验走云端 /identities/*。"""
    body = await _read_json(request)
    if body is None:
        return _error("invalid JSON body", status=400)
...
async def _auth_unbind(request: web.Request) -> web.Response:
    """解绑一种登录方式 (R2)。云端拦截「解绑最后一个身份」。"""
    provider = request.match_info.get("provider", "")
    status, data = await _auth(request).unbind(provider)
    return _auth_reply(status, data)
...
async def _auth_complete(request: web.Request) -> web.Response:
    ...
    if body is None:
        return _error("invalid JSON body", status=400)
    status, data = await _auth(request).complete(
        temp_token=str(body.get("tempToken", "")),
