...
├── AuthManager        — 登录态持有者 + 云端认证客户端（`--auth-endpoint` 为空则不创建）
...
| `_auth_manager.py` | `AuthManager` — 登录态持有者 + 云端认证服务 aiohttp 客户端（发码 / 校验 / me / logout / devices），持有 `device_key`；**不持任何供应商密钥**（安装包里放 AK/SK 等于公开发布）。`--auth-endpoint` 为空则整套不加载 |
| `_auth_store.py` | 凭证落盘 `{appdata}/auth.enc.json`（keyring 加解密）+ `device_key` 持久化；职责同 `_state.py`，但**不进 state 快照**（那里的 api_key 是明文，凭证不再踩这个坑）。钥匙串不可用时降级到明文并告警，不静默 |
...
8.5 创建 AuthManager（仅当 `--auth-endpoint` 非空；从 `{appdata}/auth.enc.json` 恢复登录态）— 为空则跳过，`/auth/*` 一条都不注册
9. await create_app(..., default_agent=..., default_workspace=..., appdata=..., schedm=..., authm=...)  — 注册 REST（含 `GET /defaults`）
...
| GET | `/auth/status` | 当前登录态 `{endpoint, loggedIn, deviceKey, platform, credentialEncrypted}`；**不含 token**。未配 `--auth-endpoint` 时本路由不存在 |
| POST | `/auth/send-code` | 请云端发验证码 `{phone?, email?, invitationCode?}`；二者都缺 → 400；云端不可达 → 502 |
| POST | `/auth/verify` | 校验验证码 `{code, phone?, email?}`；老用户直接登录并落盘凭证，新用户返回 `{tempToken, isNewUser}`。转发时自动带上 `deviceKey` / `platform` |
| POST | `/auth/complete` | 两段式注册第二段 `{tempToken, displayName?, invitationCode?}` → 建号并落盘正式 token |
| GET | `/auth/me` | 转发云端 `/me`；云端 401 时**如实转发**并清本机凭证（不假装已登录） |
| POST | `/auth/logout` | 撤销本会话；云端不可达时也清本机（否则界面骗人） |
| GET | `/auth/devices` | 已登录设备列表（R5） |
| DELETE | `/auth/devices/{device_id}` | 踢掉某台设备，该设备下次请求即 401（R5） |
