...
export async function verify(body: {
  ...
  phone?: string
  email?: string
}): Promise<{ token?: string; tempToken?: string; isNewUser?: boolean; user?: AuthUser }> {
  await sleep(500)
  if (body.code !== MOCK_CODE) {
    attemptsLeft -= 1
    if (attemptsLeft <= 0) fail('rate_limited') // 次数耗尽 → 校验侧限频（D2）
    fail('invalid_code', { remaining: attemptsLeft })
  }
  ...
  return { tempToken: pendingTemp, isNewUser: true }
}
...
export async function bind(body: { code: string; phone?: string; email?: string }): Promise<{ ok: boolean }> {
  await sleep(400)
  if (body.code !== MOCK_CODE) fail('invalid_code', { remaining: 4 })
