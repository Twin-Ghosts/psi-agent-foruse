...
describe('错误码归屏', () => {
  ...
  it('码错类归 D1，就地提示不跳屏（跳屏会丢用户已填内容）', () => {
    expect(failScreenFor('invalid_code')).toBe('D1')
    expect(failScreenFor('code_expired')).toBe('D1')
  ...
  it('tempToken 过期可识别，用于退回 A1 而非停在 A3 死等', () => {
    ...
    expect(isTempTokenExpired('invalid_code')).toBe(false)
  })
})
...
describe('从错误对象取限频字段', () => {
  ...
  it('remaining 为 0 要能取到（0 是有效值，不能被当成缺失）', () => {
    const e = Object.assign(new Error('invalid_code'), { remaining: 0 })
