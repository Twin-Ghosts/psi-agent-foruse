...
def test_compile_time_guard(iss_text: str) -> None:
    ...
    assert re.search(r'FileSize\("haitun\.exe"\)\s*<\s*\d+', iss_text), (
        "缺编译期护栏:未检查 haitun.exe 体积(防占位/桩)"
    )
...
def test_view_buttons_match_dontcopy_files(iss_text: str) -> None:
    ...
    dontcopy = set(
        re.findall(r'Source:\s*"legal\\([^"]+)";\s*Flags:\s*dontcopy', iss_text)
    )
...
def test_legal_html_self_contained(path: Path) -> None:
    ...
    assert not re.search(r'src\s*=\s*["\']https?://', text), (
        f"{path.name} 含联网加载的资源(src)，应自包含"
    )
    assert not re.search(r'href\s*=\s*["\']https?://', text), (
        f"{path.name} 含外链样式/资源(href)，应自包含"
    )
...
def test_real_compile_rejects_placeholder(tmp_path: Path) -> None:
    ...
    try:
        r = subprocess.run(
            ...
            capture_output=True, text=True, cwd=str(INNO_DIR), timeout=120,
        )
...
def test_real_compile_succeeds_with_valid_exe(tmp_path: Path) -> None:
    ...
    try:
        ...
        r = subprocess.run(
            [ISCC_PATH, f"/O{tmp_path}", str(ISS)],
            capture_output=True, text=True, cwd=str(INNO_DIR), timeout=180,
        )
